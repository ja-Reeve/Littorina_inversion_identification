...
### List of inversions
INVs <- c("LGC1.2", "LGC2.1", "LGC4.1", "LGC5.1", "LGC6.1-2", "LGC7.1", 
...
PCA.plotter <- function(inversion){
  ...
  ## Download PCA data
  dat <- read.csv(paste0("PCA_per_inversion/", LG, "_PCA_of_", inversion, "_sax.csv"), header = TRUE)
...
ggarrange(plotlist = lapply(INVs, PCA.plotter), ncol = 4, nrow = 5)
