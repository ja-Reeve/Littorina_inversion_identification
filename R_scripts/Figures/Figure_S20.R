...
### List of inversions
# Note: LGC5.1, LGC14.2 and LGC14.3 dropped due to uncertain clustering
INVs <- c("LGC1.1", "LGC1.2", "LGC2.1", "LGC4.1", "LGC6.1-2", "LGC7.1", "LGC7.2", 
          "LGC9.1", "LGC9.2", "LGC10.1", "LGC10.2", "LGC11.1", "LGC12.1", "LGC12.2", 
          "LGC12.3", "LGC12.4", "LGC14.1", "LGC17.1")

### Set colour and shape scales
# Colour by genotype
K3_col <- c("#1f78b4", "#a4db77ff", "#e31a1c")
K6_col <- c("#1f78b4", "#a6cee3", "#33a02c", "#fb9a99", "#e31a1c")
# Shape by ecotype
EC_col <- c(1, 17, 15, 18, 19)
...
PCA.plotter <- function(inversion){
  ...
  ## Download PCA data
  dat <- read.csv(paste0("PCA_per_inversion/", LG, "_PCA_of_", inversion, "_v2_arc-sax.csv"), header = TRUE)
...
figS20 <-ggarrange(plotlist = lapply(INVs, PCA.plotter), ncol = 4, nrow = 5)

### Save plot
tiff("plots/Supp_mat_plots/FigS20.tiff", width = 42, height = 52, units = "cm", res = 300)
figS20
dev.off()
