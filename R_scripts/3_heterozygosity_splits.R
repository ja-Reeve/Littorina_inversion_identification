...
### Edited 23/02/2023: clean up code for publication

### 1. Preparation ####
### clear the environment
rm(list = ls())
dev.off()
setwd("/Users/james/Documents/Inversion_detection/Heterozygosity_scores")
...
# Northern saxatilis
Het_NS <- read.csv(paste0(LG.s,"_Het_v2_NS.csv"))
Het_NS <- Het_NS[Het_NS$Nsnp >= 10 & Het_NS$nHe > 0,]

# Spain
Het_Sp <- read.csv(paste0(LG.s,"_Het_v2_Sp.csv"))
Het_Sp <- Het_Sp[Het_Sp$Nsnp >= 10 & Het_Sp$nHe > 0,]

# Littorina arcana
Het_arc <- read.csv(paste0(LG.s,"_Het_v2_arc.csv"))
...
lm_max <- ceiling(max(c(max(Het_NS$avgMP), max(Het_Sp$avgMP), max(Het_arc$avgMP)))) # max position (rounding up)
mp_winds <-  seq(1, lm_max, 1) # set of possible step positions

# individuals
NS_IDs <- as.character(unique(Het_NS$SnailIDs))
...
# Northern saxatilis
Hsplit_NS <- lapply(NS_IDs, function(i){
  He.nest.split(data = Het_NS, snail.ID = i, linkageMap.cuts = mp_winds, alpha = Alpha)})
Hsplit_NS <- do.call(rbind.data.frame, Hsplit_NS)
...
# Save results
write.csv(Hsplit_NS, paste0("/Users/james/Documents/Inversion_detection/split_function_results/Hsplit_betaBin_", LG.s, "_v2_NS.csv"), row.names = F, quote = F)
write.csv(Hsplit_Sp, paste0("/Users/james/Documents/Inversion_detection/split_function_results/Hsplit_betaBin_", LG.s, "_v2_Sp.csv"), row.names = F, quote = F)
write.csv(Hsplit_arc, paste0("/Users/james/Documents/Inversion_detection/split_function_results/Hsplit_betaBin_", LG.s, "_v2_arc.csv"), row.names = F, quote = F)
...
split_scores_NS <- lapply(NS_IDs, function(i){
  extract.heterzygosities(X = Hsplit_NS[Hsplit_NS$SnailID == i,])})
...
split_scores_NS <- do.call(rbind.data.frame, split_scores_NS)
...
pNS <- plot.split.function(Het_NS, Hsplit_NS, split_scores_NS, "Northern saxatilis", Alpha)
...
ggsave(paste0(LG.s, "hetSplit_v2_NS.v4.tiff"), pNS, "tiff", PATH, width = 30, height = 15, units = "cm")
ggsave(paste0(LG.s, "hetSplit_v2_Sp.v4.tiff"), pSp, "tiff", PATH, width = 30, height = 15, units = "cm")
ggsave(paste0(LG.s, "hetSplit_v2_arc.v4.tiff"), parc, "tiff", PATH, width = 30, height = 15, units = "cm")
