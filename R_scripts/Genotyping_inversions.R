...
### PC2 are used for clustering. Finally, clusters were relabelled to 
### consistently so 'R' matches the cluster with the highest proportion
### of crab individuals.
...
inversion.genotype <- function(inversion, cohort){
  ### A: Access PCA results
  # Get focal linkage group from inversion name
  LG <- gsub("C", "", gsub("[.].*", "", inversion))
  # Upload the PCA
  PC <- read.csv(paste0(LG, "_PCA_of_", inversion, "_", cohort, ".csv"))
  
  if(inversion == "LGC6.1-2"){colnames(PC) <- gsub("LGC6.1.2.", "", colnames(PC))
  } else {
    colnames(PC) <- gsub(paste0(inversion, "."), "", colnames(PC))
  }
  ...
  if(K == 6){
    ...
    clust_F <- which.max(bestK$centers[,1]) # Search PC1 for maximum centre value
    clust_C <- which.max(bestK$centers[,2]) # Search PC2 for maximum centre value
...
Nsax <- lapply(INVs, inversion.genotype, cohort = "sax")
...
Ssax <- lapply(INVs, inversion.genotype, cohort = "Sp")
names(Ssax) <- INVs
...
annotate_figure(ggarrange(plotlist = SsaxP, common.legend = TRUE, legend = "right"), 
                top = text_grob("Spanish Littorina saxatilis", size = 24),
                left = text_grob("PC2", size = 18, face = "bold", rot = 90), 
                bottom = text_grob("PC1", size = 18, face = "bold"))

#### Plot Littorina arcana ####
### Note: this deosn't work, as there wer no 'crab' ecotypes in Larc
...
LarcSax <- lapply(INVs, inversion.genotype, cohort = "arc-sax")
names(LarcSax) <- INVs
...
sapply(names(Nsax), function(inv){
  LG <- gsub("C", "", gsub("[.].*", "", inv))
  write.csv(Nsax[inv], paste0(LG, "_PCA_of_", inv, "_sax.csv"),
...
sapply(names(Ssax), function(inv){
  LG <- gsub("C", "", gsub("[.].*", "", inv))
  write.csv(Ssax[inv], paste0(LG, "_PCA_of_", inv, "_Sp.csv"),
            quote = FALSE, row.names = FALSE)
})
...
sapply(names(Larc), function(inv){
  LG <- gsub("C", "", gsub("[.].*", "", inv))
  write.csv(Larc[inv], paste0(LG, "_PCA_of_", inv, "_arc.csv"),
            quote = FALSE, row.names = FALSE)
})
...
sapply(names(LarcSax), function(inv){
  LG <- gsub("C", "", gsub("[.].*", "", inv))
  write.csv(LarcSax[inv], paste0(LG, "_PCA_of_", inv, "_arc-sax.csv"),
            quote = FALSE, row.names = FALSE)
})
...
sapply(names(ALL), function(inv){
  LG <- gsub("C", "", gsub("[.].*", "", inv))
  write.csv(ALL[inv], paste0(LG, "_PCA_of_", inv, "_all.csv"),
            quote = FALSE, row.names = FALSE)
})
